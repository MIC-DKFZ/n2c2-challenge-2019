...
def optimizing_bert_results(sts_data_train_path, sts_data_test_path, output_folder):

    pickle_folder_train = os.path.join(output_folder, 'train')
    pickle_folder_test = os.path.join(output_folder, 'test')
    ...
    experiments_schedule = [
        ...
        {
            'measures': measure_bert + measures_textdistance3 + measures_textdistance4 + pooled_sentence_measures +
                         infersent_1_sentence_measures,
            'estimator': estimator,
        },
        # {
    ...
    for exp_idx, experiment_config in enumerate(experiments_schedule):
        ...
        pearson_list_dict = {'Average pearson': [total_dev_pearson], 'Rund idx': exp_idx}
        ...
        final_prediction.to_csv(os.path.join(output_folder, f"{exp_idx}_dev_prediction.csv"),
        ...
        final_prediction.to_csv(os.path.join(output_folder, f"{exp_idx}_test_prediction.csv"),
                                header=False, index=False)
...
if __name__ == '__main__':
    ...
    output_folder = os.environ.get('OUTPUT_FOLDER', os.path.join(NLP_EXPERIMENT_PATH, 'pickles_for_opt'))
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    optimizing_bert_results(sts_data_train_path, sts_data_test_path, output_folder)
